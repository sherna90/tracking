...
vector<Rect> CPU_HMC_HOGDetector::detect(Mat &frame, vector<Rect> samples)
{
...
void CPU_HMC_HOGDetector::train(Mat &frame,Rect reference_roi)
{
	...
	if(!this->hmc.initialized){
		this->hmc.init(this->feature_values, this->labels, args.lambda, 100, args.n_iterations, 0.01, 10);	
	} 
	else{
...
void CPU_HMC_HOGDetector::samplerBox(Mat &current_frame, Rect ground_truth, int n_particles, vector<Rect>& sampleBox, vector<Rect>& negativeBox){
