...
vector<Rect> Mask_GPU_HMC_HOGDetector::detect(Mat &frame)
{
	Mat current_frame;
...
void Mask_GPU_HMC_HOGDetector::train(Mat &frame,Rect reference_roi)
{
	Mat current_frame;
	frame.copyTo(current_frame);
	if(args.resize_src) {
...
MatrixXd Mask_GPU_HMC_HOGDetector::getFeatureValues(Mat &frame)
{
...
void Mask_GPU_HMC_HOGDetector::train()
{
	this->hmc.init(this->feature_values, this->labels, args.lambda, 100, args.n_iterations, 0.01, 100);
	this->hmc.run();
...
VectorXd Mask_GPU_HMC_HOGDetector::predict(MatrixXd data)
{
	return this->hmc.predict(data, false);
}

void Mask_GPU_HMC_HOGDetector::loadModel(VectorXd weights,VectorXd featureMean, VectorXd featureStd, VectorXd featureMax, VectorXd featureMin, double bias){
