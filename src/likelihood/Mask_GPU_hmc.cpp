...
#include "Mask_GPU_hmc.hpp"

void Mask_GPU_Hamiltonian_MC::init(MatrixXd &_X, VectorXd &_Y, double _lambda, int _warmup_iterations, int _iterations, double _step_size, int _num_step, double _path_length){
	this->lambda=_lambda;
...
VectorXd Mask_GPU_Hamiltonian_MC::gradient(VectorXd &W){
...
double Mask_GPU_Hamiltonian_MC::logPosterior(VectorXd &W, bool precompute){
	double logPost = 0.0;
	if (this->init_hmc){
		VectorXd temp = W.tail(W.rows()-1);
		this->logistic_regression.setWeights(temp);
...
void Mask_GPU_Hamiltonian_MC::run(bool warmup_flag){
	if (!warmup_flag) cout << "Run" << endl;
	if (this->init_hmc){	

		VectorXd mask = VectorXd::Ones(this->dim-1);
		...
		while (n < this->iterations){
			...
			if (this->with_mask) this->logistic_regression.setTrainMask(mask);
			...
			if (n>=0){
				...
				if (this->with_mask) mask_matrix.row(n) = mask;
			}
			//if (this->with_mask and !accepted_flag) mask = this->random_binomial(1, VectorXd::Ones(this->dim -1)/2.0, this->dim -1);
			if (this->with_mask) mask = this->random_binomial(1, VectorXd::Ones(this->dim -1)/2.0, this->dim -1);
		...
		if (!warmup_flag){
			...
			//if (this->with_mask) tools.writeToCSVfile("hmc_mask.csv", this->mask_matrix);
		}
		else{
...
VectorXd Mask_GPU_Hamiltonian_MC::predict(MatrixXd &X_test, bool prob, int samples, bool erf, bool prob_label){
	/*
	Predict: 
	...
	*/
	...
	if (this->init_hmc){
		...
		if (samples == 0){
			if (this->with_mask){
				/*VectorXd mean_mask =(this->mask_matrix.colwise().mean()).unaryExpr([](double elem) {
				    return round(elem);
				});*/
				VectorXd temp_mask(X_test.rows());
				VectorXd mean_mask =(this->mask_matrix.colwise().mean());
				temp_mask =  this->random_binomial(1, mean_mask, mean_mask.rows());

				this->logistic_regression.setTestMask(mean_mask);
			}
...
MatrixXd Mask_GPU_Hamiltonian_MC::get_maskMatrix(){
	if (this->init_hmc){
...
void Mask_GPU_Hamiltonian_MC::set_maskMatrix(MatrixXd &_mask_matrix){
	if (this->init_hmc){
...
void Mask_GPU_Hamiltonian_MC::getModel(VectorXd& weights, VectorXd& featureMean, VectorXd& featureStd, VectorXd& featureMax, VectorXd& featureMin, double& bias){
...
void Mask_GPU_Hamiltonian_MC::loadModel(VectorXd weights, VectorXd featureMean, VectorXd featureStd, VectorXd featureMax, VectorXd featureMin, double bias){
...
void Mask_GPU_Hamiltonian_MC::setData(MatrixXd &_X,VectorXd &_Y){
